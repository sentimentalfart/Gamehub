...
// --- Creator Studio: Export Game Config ---
function generateAndDownloadGame() {
  const gameConfig = {
    ...
    playerColor: document.getElementById('custom-color').value
  };

  // Convert settings to a downloadable JSON file
  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(gameConfig, null, 2));
  const downloadAnchor = document.createElement('a');
  downloadAnchor.setAttribute("href", dataStr);
  downloadAnchor.setAttribute("download", `${gameConfig.title.toLowerCase().replace(/\s+/g, '_')}.json`);
...
document.getElementById('mod-upload').addEventListener('change', function(e) {
  ...
  reader.onload = function(evt) {
    try {
      ...
      // Apply custom settings into the running engine
      window.activeGameConfig = config;
      alert(`Successfully loaded game preset: "${config.title}"!`);
    } catch (err) {
      alert("Invalid JSON game file. Please upload a file generated by the Creator Studio.");
    }
  };
  reader.readAsText(file);
});
